...
namespace Polynomials
{
    ...
    public class SymmetricPolynomial
    {
        ...
        public static SymmetricPolynomial PowerPrime(int k)
        {
            ...
            {
                output += new BigRational(IntegerFunctions.Moebius(k / d), 1) * Power(d);
            }
        ...
        private static IEnumerable<List<int>> CoefTermsEnumerator(int r, bool ByTerms = true)
        {
            ...
            while (true)
            {
                ...
                foreach (List<int> l in IntegerFunctions.AllPartitions(n))
                {
                    k++;
                    yield return l;
                }
        ...
        private static BigInteger MultinomialOfPartition(List<int> p, int n)
        {
            ...
            BigInteger result = IntegerFunctions.Factorial(n) / IntegerFunctions.Factorial(n - p.Count);
            int k = 1;
            for (int i = 1; i < p.Count; i++)
            {
                ...
                {
                    result /= IntegerFunctions.Factorial(k);
                    k = 1;
                }
            }
            result /= IntegerFunctions.Factorial(k);
